...
import { colVisible, fieldVisible } from "./useColumnVisibility.js?v=12";
import {
  buildFallbackIntegrationCards,
  cachedAll,
  diagnosticsFromGeneric,
  hasRenderableIntegrationCardData,
  stringArray,
} from "./genericScanResultUtils.js?v=12";
...
function adaptGenericIPResult(result) {
  ...
  return {
    ...
    geo: {
      ...
      crawlerString: ipapi.crawlerString || "",
...
export const activeResultEntry = computed(
  () => allResults.value[activeIdx.value] || null,
);
export const activeResult = computed(() => {
  const raw =
    activeResultEntry.value?.result || activeResultEntry.value || null;
...
export const activeIPAPIManifestCard = computed(() => {
  ...
  const synthesizedResult = rawResult || {
    ...
    crawlerString: geo.crawlerString || "",
  ...
  return {
    ...
    manifest,
    ioc:
      raw?.ioc || activeResult.value?.ipAddress || activeResult.value?.ip || "",
...
export const activeIPFallbackCards = computed(() => {
  ...
  return isGenericIPResult(raw)
    ? buildFallbackIntegrationCards(
        raw,
        "ip",
        new Set([...CUSTOM_IP_CARD_NAMES, "ipapi"]),
      )
    : [];
});
...
export const networkRows = computed(() => {
  ...
  return [
    ...
    ["Crawler String", g.crawlerString || "—", "net-crawlerstring"],
...
export const TABLE_COLS = [
  ...
  },
  {
    key: "country",
    label: "Country",
    vis: "field:net-country",
    get: (d) => d.geo?.country || d.geo?.countryCode || "—",
  },
  {
    key: "city",
    label: "City",
  ...
  },
  {
    ...
    label: "Timezone",
    vis: "field:net-tz",
    get: (d) => d.geo?.timezone || "—",
  },
  {
    key: "isPublic",
    label: "Public",
    vis: "field:net-public",
    get: (d) => d.abuseipdb?.isPublic,
  },
  {
    key: "isWhitelisted",
    label: "Whitelisted",
    vis: "field:net-wl",
    get: (d) => d.abuseipdb?.isWhitelisted,
  },
  {
    key: "hostnames",
    label: "Hostnames",
    vis: "field:net-hostnames",
    get: (d) => d.abuseipdb?.hostnames || [],
  },
  {
    key: "abuseConfidenceScore",
    label: "Abuse %",
    vis: "field:ab-score",
    get: (d) => d.abuseipdb?.confidenceScore ?? "—",
  },
  {
    key: "totalReports",
    label: "Reports",
    vis: "field:ab-reports",
    get: (d) => d.abuseipdb?.totalReports ?? "—",
  },
  {
    key: "lastReportedAt",
    label: "Last Reported",
    vis: "field:ab-lastreport",
    get: (d) =>
      d.abuseipdb?.lastReportedAt
        ? d.abuseipdb.lastReportedAt.replace("T", " ").replace("+00:00", "")
        : "—",
  },
  {
    key: "vtStats_S_U_H",
    label: "VT S/U/H",
    vis: "field:vt-summary",
    get: (d) =>
      d.virustotal
        ? `${d.virustotal.suspicious}/${d.virustotal.undetected}/${d.virustotal.harmless}`
        : "—",
  },
  {
    key: "vtMalicious",
    label: "VT Malicious",
    vis: "field:vt-malicious",
    get: (d) => d.virustotal?.malicious ?? "—",
  },
  {
    key: "vtSuspicious",
    label: "VT Suspicious",
    vis: "field:vt-suspicious",
    get: (d) => d.virustotal?.suspicious ?? "—",
  },
  {
    key: "vtReputation",
    label: "VT Reputation",
    vis: "vt",
    get: (d) => d.virustotal?.reputation ?? "—",
  },
  {
    key: "vtLastAnalysis",
    label: "VT Last Scan",
    vis: "field:vt-lastanalysis",
    get: (d) => d.virustotal?.lastAnalysisDate || "—",
  },
  {
    key: "link_virustotal",
    label: "VT Link",
    vis: "field:link-vt",
    get: (d) => d.links?.virustotal || "",
  },
  {
    key: "link_abuseipdb",
    label: "AbuseIPDB Link",
    vis: "field:link-abuse",
    get: (d) => d.links?.abuseipdb || "",
  },
  {
    key: "link_ipapi",
    label: "IPAPI Link",
    vis: "field:link-ipapi",
    get: (d) => d.links?.ipapi || "",
  },
];
...
export function isColVisible(col) {
  ...
  if (col.vis.startsWith("field:"))
    return fieldVisible[col.vis.slice(6)] !== false;
  return colVisible[col.vis] !== false;
}

export const visibleTableCols = computed(() =>
  TABLE_COLS.filter((c) => isColVisible(c)),
);

export const sortedTableRows = computed(() => {
  const rows = allResults.value
    .map((e, i) => ({
      ...adaptGenericIPResult(e.result || e),
      _ip: e.ip || e.ioc || e.ipAddress || e.ip,
      _idx: i,
    }))
...
export function sortTable(key) {
  if (tableSortCol.value === key) {
    tableSortAsc.value = !tableSortAsc.value;
  } else {
    tableSortCol.value = key;
...
export function renderTableCell(col, row, i) {
  ...
  if (col.key === "isPublic" || col.key === "isWhitelisted") {
    ...
    return val
      ? `<span class="t-bool-yes">✓ Yes</span>`
      : `<span class="t-bool-no">✗ No</span>`;
  }
...
export function extractIPs(text) {
  ...
  const found = [
    ...(normalized.match(ipv4) || []),
    ...(normalized.match(ipv6) || []),
  ];
...
export async function copyClipboard(format, copyMenuOpenRef) {
  ...
  const rows = allResults.value
    .map((e) => adaptGenericIPResult(e.result || e))
    .filter((e) => !!e);
...
function _buildFilteredRows() {
  ...
  return allResults.value
    .map((e, i) => ({ row: adaptGenericIPResult(e.result || e), idx: i }))
    .filter(({ row }) => !!row)
    .map(({ row, idx }) => {
      const obj = {};
      cols.forEach((c) => {
        ...
        if (Array.isArray(v)) v = v.join("; ");
        const exportKey =
          c.key === "link_virustotal"
            ? "virustotal_link"
            : c.key === "link_abuseipdb"
              ? "abuseipdb_link"
              : c.key === "link_ipapi"
                ? "ipapi_link"
                : c.key;
