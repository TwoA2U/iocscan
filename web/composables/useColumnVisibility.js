...
export const fieldVisible = reactive({
  ...
  "net-localtime": true,
  "net-company": true,
  "net-companytype": true,
  "net-companydomain": true,
  "net-abuserscore": true,
  "net-vpnservice": true,
  "net-vpntype": true,
  "net-vpn": true,
  "net-proxy": true,
...
export const HASH_COL_LABELS = {
  ...
  vtHarmless: "VT Harmless",
  vtUndetected: "VT Undetected",
  vtReputation: "VT Reputation",
  suggestedThreatLabel: "Threat",
  popularThreatCategories: "VT Categories",
  popularThreatNames: "VT Names",
  mbQueryStatus: "MB Status",
  mbSignature: "MB Signature",
  mbFileName: "MB File Name",
  mbFileType: "MB Type",
  mbTags: "MB Tags",
  mbComment: "MB Comment",
  signatureSigners: "Code Signer",
  sigmaAnalysisSummary: "Sigma",
  sandboxMalwareClassifications: "Sandbox",
  signerDetail: "Signer Detail",
  md5: "Md5",
  sha1: "Sha1",
  sha256: "Sha256",
  link_virustotal: "VT Link",
  link_malwarebazaar: "MB Link",
};
...
export const HASH_SECTION_DEFS = [
  ...
  },
  {
    ...
    icon: "🔗",
    label: "Links",
    keys: ["link_virustotal", "link_malwarebazaar"],
  },
];
...
export function toggleSection(key) {
  ...
  if (SECTION_FIELDS[key]) {
    SECTION_FIELDS[key].forEach((f) => {
      fieldVisible[f] = colVisible[key];
    });
  }
}
...
export function toggleHashCol(key) {
  ...
  if (c) {
    c.visible = !c.visible;
    hashColVisible[key] = c.visible;
  }
}

export function toggleHashSection(secKey) {
  const KEYS = Object.fromEntries(
    HASH_SECTION_DEFS.map((d) => [d.key, d.keys]),
  );
  let cols;
  if (secKey === "hother") {
  ...
  cols.forEach((c) => {
    c.visible = newVal;
    hashColVisible[c.key] = newVal;
  });
}

export function setAllCols(visible, currentIOCMode) {
  if (currentIOCMode === "hash") {
    hashDynCols.forEach((c) => {
      c.visible = visible;
      hashColVisible[c.key] = visible;
    });
  } else {
    Object.keys(colVisible).forEach((k) => {
      colVisible[k] = visible;
    });
    Object.keys(fieldVisible).forEach((f) => {
      fieldVisible[f] = visible;
    });
  }
}
...
export function buildHashDynCols(results) {
  ...
  results.forEach((e) => {
    ...
    if (vt.popularThreatCategories?.length)
      hasData.add("popularThreatCategories");
    if (vt.popularThreatNames?.length) hasData.add("popularThreatNames");
    if (vt.sandboxMalwareClassifications?.length)
      hasData.add("sandboxMalwareClassifications");
  ...
  hashDynCols.splice(
    ...
    ...ordered.map((key) => {
      ...
      const defaultVisible =
        hasData.has(key) && !HASH_COL_HIDDEN_DEFAULT.has(key);
      const visible = prev
        ? prev.visible
        : hashColVisible[key] !== undefined
          ? hashColVisible[key]
          : defaultVisible;
...
export function makeColBadge(currentIOCMode) {
  ...
  const allToggs = [
    ...Object.values(colVisible),
    ...Object.values(fieldVisible),
  ];
...
export function makeIpDrawerSections(allResults, activeIdx) {
  ...
  const sections = [];
  if (d.riskLevel != null)
    sections.push({ key: "risk", icon: "🏷", label: "Risk Pill", fields: [] });
  const netFields = [
    ...
    { key: "net-isp", label: "ISP", present: !!(d.geo?.isp || d.geo?.asn_org) },
    {
      key: "net-country",
      label: "Country",
      present: !!(d.geo?.country || d.geo?.countryCode),
    },
    ...
    { key: "net-company", label: "Company", present: !!d.geo?.companyName },
    {
      key: "net-companytype",
      label: "Company Type",
      present: !!d.geo?.companyType,
    },
    {
      key: "net-companydomain",
      label: "Company Domain",
      present: !!d.geo?.companyDomain,
    },
    {
      key: "net-abuserscore",
      label: "Abuser Score",
      present: !!d.geo?.abuserScore,
    },
    {
      key: "net-vpnservice",
      label: "VPN Service",
      present: !!d.geo?.vpnService,
    },
    ...
    { key: "net-tor", label: "Tor", present: !!d.geo?.isTor },
    {
      key: "net-datacenter",
      label: "Datacenter",
      present: !!d.geo?.isDatacenter,
    },
    { key: "net-crawler", label: "Crawler", present: !!d.geo?.isCrawler },
    {
      key: "net-crawlerstring",
      label: "Crawler String",
      present: !!d.geo?.crawlerString,
    },
    ...
    { key: "net-public", label: "Public", present: d.geo?.isPublic != null },
    {
      key: "net-wl",
      label: "Whitelisted",
      present: d.geo?.isWhitelisted != null,
    },
  ...
  const abuseFields = [
    {
      ...
      present: d.abuseipdb?.confidenceScore != null,
    },
    {
      key: "ab-meter",
      label: "Score Meter",
    ...
    },
    {
      key: "ab-reports",
      label: "Total Reports",
      present: d.abuseipdb?.totalReports != null,
    },
    {
      key: "ab-lastreport",
      label: "Last Reported",
      present: !!d.abuseipdb?.lastReportedAt,
    },
  ].filter((f) => f.present);
  if (abuseFields.length)
    sections.push({
      key: "abuse",
      icon: "🚨",
      label: "AbuseIPDB",
  ...
  const vtFields = [
    {
      key: "vt-malicious",
      label: "🔴 Malicious",
      present: d.virustotal?.malicious != null,
    },
    {
      key: "vt-suspicious",
      label: "🟡 Suspicious",
      present: d.virustotal?.suspicious != null,
    },
    { key: "vt-harmless", label: "🟢 Harmless", present: d.virustotal != null },
    {
      key: "vt-undetected",
      label: "⬜ Undetected",
      present: d.virustotal != null,
    },
    {
      key: "vt-summary",
      label: "Summary S/U/H",
    ...
    },
    {
      key: "vt-lastanalysis",
      label: "Last Scan",
      present: !!d.virustotal?.lastAnalysisDate,
    },
  ].filter((f) => f.present);
  if (vtFields.length)
    sections.push({
  ...
  const linkFields = [
    {
      key: "link-vt",
      label: "VirusTotal Link",
      present: !!d.links?.virustotal,
    },
    {
      key: "link-abuse",
      label: "AbuseIPDB Link",
      present: !!d.links?.abuseipdb,
    },
...
export function makeHashDrawerSections() {
  ...
  if (otherCols.length) {
    result.push({
      ...
      label: "Other",
      cols: otherCols,
      allOn: otherCols.every((c) => c.visible),
    });
  }
  return result;
}
